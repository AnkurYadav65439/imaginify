...
export async function addImage({ image, userId, path }: AddImageParams) {
    try {
        ...
        console.log("error for add image ", error);
        handleError(error);
    }
}
...
export async function updateImage({ image, userId, path }: UpdateImageParams) {
    try {
        ...
        const updatedImage = await Image.findByIdAndUpdate(imageToUpdate._id, { ...image, updatedAt: new Date() }, { new: true });
...
export async function getAllImages({ limit = 9, page = 1, searchQuery = '' }: { limit?: number, page: number, searchQuery: string }) {
    try {
        ...
        //should be same a your cloudinary folder name
        ...
        //fetching from our backend after cloudinary fetching
        ...
        const savedImages = await Image.find().countDocuments();   //total images in general
